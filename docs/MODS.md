...
- 上传正常 `.pak` 文件后，MOD 会出现在列表中。
- `.pak` 可以在“启用”和“禁用”之间切换。
- 删除 MOD 会先移动到废纸篓，不会立即物理删除。
- 废纸篓可以手动清理。
- 包含官方 `Info.json` 的 `.zip` 可以被识别并导入。
- 空 `.pak/.sig` 文件会被拒绝。
- `.sh/.ps1/.bat/.cmd/.exe/.dll/.so` 等脚本或高风险文件会被拒绝。
- zip 内部的路径穿越、符号链接和危险文件会被拒绝。

这些验证只代表面板的文件管理流程正常，不代表每一个第三方 MOD 都能在 Palworld Linux/Docker 服务端环境中生效。
...
5. 上传完成后，先只启用一个 MOD 进行测试。
6. 点击 `应用并重启`，面板会先尝试备份当前存档，再重启 Palworld。
7. 进入 `实时日志` 和游戏客户端确认服务器是否正常启动、能否进服。
8. 如果异常，回到 `MOD 管理` 禁用最近添加的 MOD，再次应用并重启。

建议不要一次性启用多个新 MOD。多个 MOD 同时出问题时，很难判断是哪一个导致崩服或无法进服。
...
`imports` 是上传导入记录目录。删除 MOD 后，列表里不会再显示该 MOD，但导入记录可能仍保留，方便排查来源和历史操作。
...
如果禁用 MOD 后仍无法恢复，建议按这个顺序排查：

1. 查看 `实时日志` 里最后的错误。
2. 确认客户端是否也安装了同款 MOD。
3. 禁用最近新增的所有 MOD。
4. 使用 `存档管理` 恢复安装 MOD 前的备份。
5. 确认 Palworld 版本和 MOD 作者标注的兼容版本是否一致。
